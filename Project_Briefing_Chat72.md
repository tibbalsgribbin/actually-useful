...
*Volatile state last updated: Chat 72 (Phase 5 settings page — end of Phase 4+5 bundle) — see PART TWO for current versions*
...
`chrome.storage.local` key `au_telemetry_enabled`. Default: opted in (`true`). User can opt out via the Settings page (§7.4) — toggle wired in Phase 5. background.js checks this key before forwarding log payloads.
...
*Phase 4+5 bundle complete as of Chat 72.*

---

## 14. Current versions (as of Chat 72)

- **Overall / canonical version:** v0.6.1.83 (search.js)
- **manifest:** v0.6.1
- **search.js:** v0.6.1.83
- **core.js:** v0.6.1.53
...
**Active count pill:** Shows count of all non-default filter states. As of Phase 5 (Chat 72), comparisons are against `userDefaults.*` (user-saved defaults from `chrome.storage.local`), falling back to built-in defaults when no user default is set. Fields without user-default surfaces (price range, sources, badges, keyword) still use hardcoded comparison.
...
**Pages default:** `userDefaults.pages` (default 4). Panel auto-fetches this many pages on every build. User can change in Settings. Stored in `auDefaultPages`.
...
**Card padding:** dense default `8px 14px`. Comfortable variant `16px 14px`. Density class applied to `#ppu-list` at render time (`.density-dense` or `.density-comfortable`).
...
- Clicking ⋯ opens a small popover anchored below the button with two actions:
...
- Row hidden entirely if no brand was detected

**Card density preference:** Storage key `auCardDensity`. Values: `'dense'` (default) or `'comfortable'`. Loaded at startup via `loadCardDensity(cb)`. UI to change it: Settings page §7.1 radio (Phase 5, Chat 72). Also offered during onboarding wizard (Phase 6).
...
**Minimize/expand:**
- `#ppu-minimize` button (− in expanded header) wired; double-click title bar also works
...
**Drag:** title bar is drag handle in both expanded and minimized states. Icon buttons stop mousedown propagation. Click vs drag disambiguation: 4px / 200ms threshold.

**Resize:** left-edge handle (`#ppu-drag-handle`). Width clamped **320–600px**. `fixedRight` clamped to `window.innerWidth - 4` on mousedown to prevent off-screen push when narrowing. Handle stays on left edge of panel always.
...
**State machine:** `panelInSettings` boolean. `openSettings()` hides content rows below the header (header stays live for drag/minimize), inserts `#ppu-settings-view` before footer, swaps gear to back arrow, widens panel to 580px if narrower. `closeSettings()` reverses all of this. Does not persist across page reloads.

**Save behavior:** instant save on every control change. No Save button. Back arrow is pure navigation.

**Four sections:**

§7.1 Defaults for every search: Default sort (`auDefaultSort`), Pages to load (`auDefaultPages`), Move ads to end (`auDefaultMoveAdsToEnd`), Card density (`auCardDensity`)

§7.2 Quality thresholds: Minimum rating (`auDefaultMinRating`), Minimum reviews (`auDefaultMinReviews`)

§7.3 Brand & shipping: Move Amazon brands to end (`auDefaultMoveAmazonBrands`), Move unrecognized brands to end (`auDefaultMoveUnrecognized`), Hide slow shipping + days (`auDefaultHideSlowShipping`, `auDefaultSlowShippingDays`)

§7.4 Privacy: Share anonymous usage data (`au_telemetry_enabled` — existing key, now wired to toggle)

**Reset to defaults:** two-click confirmation, 3-second revert. Clears all `auDefault*` keys plus `auCardDensity` and `au_telemetry_enabled`. Re-opens settings fresh.

**Version display:** `chrome.runtime.getManifest().version`.

**Storage keys (Phase 5, all new except noted):**
...
| `auCardDensity` | string | `'dense'` (existing key, now has UI) |

**Startup chain:** `loadUserDefaults(cb)` added after `loadPanelMinimized`, before `tryBuild`.

**Fresh search defaults:** Sort, minReviews, minRating, sponsoredMode, brandFilterActive, amazonBrandsDemoteActive, deliveryFilterActive, deliveryFilterDays all initialize from `userDefaults` on every panel load. Session filter restore applies only to true per-search overrides (keyword, price range, selectedUnit, srcFilter).
...
`isMultiPackWeight(title)` — returns true when it's safe to multiply a detected weight by a pack count. Fires when EITHER: (C) a container word appears adjacent to the weight in the title, OR (B) a strong substance/food keyword appears anywhere in the title.
...
**Phase 6 of redesign:** Full content rewrite reflecting the four-step shopping flow (Step 0 prologue + Expand/Narrow/Decide cards), brand-controls explainer, optional Personalize wizard link.

---

## Panel Redesign — phase status (as of Chat 72)
...
`Project_Briefing.md` and `Roadmap.md` use the `_Chat[N].md` filename convention going forward — same as Handover. Each version gets a unique filename so old versions are never overwritten and chat-of-origin is always visible. Coding files and uniquely-named docs (specs, mockups, briefs) are out of scope for this rule.
