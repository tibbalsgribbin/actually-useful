...
*Current version: v0.6.1.45 (overall) · v0.6.1 (manifest) · v0.6.1.45 (search.js) · v0.6.1.30 (compare.html) · Updated May 3, 2026 (Chat 44)*
...
**Shortlist item object shape sent to compare.html (current — v0.6.1.30):**
```
...
**Kill switch:** `killswitch.json` in repo root → served at `https://actuallyuseful.net/killswitch.json`. Extension fetches this on every load. Set `"disabled": true` to halt the extension for all users instantly; set a `"message"` to display a reason. Fail-open: if fetch fails or times out (3s), extension runs normally.
...
| File | Purpose |
|---|---|
| `killswitch.json` | Kill switch config — served via GitHub Pages at actuallyuseful.net/killswitch.json |
...
- **Solid product override:** when title contains pod/sheet/strip/load/pac/fling/tab/toothpaste/tooth paste keywords and Amazon reports a weight unit or whole-package $/ct, calculates price/count instead. Uses fuzzy 1% comparison (not exact ===) to catch laundry sheets and Tide Pods showing full package price as per-unit.
- **solidUnitIsWrong also covers liquid units:** if Amazon reports liquid volume (fl oz/ml) for a solid product (pods, tabs), this is also flagged — fixes Tide Pods showing $/fl oz
- **Weight-from-title fallback:** when ap=null and count=null, parses oz/lb/g/kg from title and computes $/unit directly — fixes single-bag rice and cat food showing $/ct
- **Weight sanity check:** if Amazon's $/unit × weight-in-title ≠ price by >10%, recalculates — fixes sushi rice $5.00/oz, dog food $79.92/lb
- **Weight unit normalization:** `inferWeightDominant()` detects when results mix oz and lb; unit pills offer oz/lb/g/kg for display; sort always normalizes weight units to oz via `normalizePPUForSort()`; status message "weight mix — click 'oz' to compare all items in the same unit" appears when active
- **Footage support:** "25ft", "50 FT" extracted from titles; $/ft calculated and displayed
- **normalizeUnit** strips leading numbers ("100 sheets" → "sheet") and "X per Y" suffixes ("pack per load" → "pack")
...
- **PPU zero-pad:** $0.1/ct displays as $0.10/ct
- **Sub-penny PPU:** 3 decimal places when PPU < $0.10 (e.g. $0.009/ct, $0.028/sheet)
...
- **Page limit confirmed by testing:** 7 pages maximum for high-density searches (~48 results/page); up to 10 pages for low-density. Amazon is the ceiling. Slider max = 7.

### Keyword filtering
- Inclusion, exclusion (hyphen/minus in front of word), OR branches
...
- Sort options: Best value, Lowest price, Fastest delivery of any type, Fastest free delivery, As shown in Amazon results

### Rating/review display
Rating (stars) and review count shown in each panel row, below delivery info.

### Results summary line
Shows `X/Y have unit data` plus active filter counts: pages, keyword matches, source-hidden, showing-in-unit, liquid/weight category, ads demoted/hidden, below min reviews, below min rating, outside price range, **hidden by badge filter** (new v0.6.1.44).
...
- "SNAP EBT only" filter on compare.html — only shown when at least one item is SNAP-eligible
- Verified working on real grocery searches ✅
...
- Panel note lines per item when detected
- All three in compare payload; pills shown in Coupon/promo column; conditional filters in compare filter bar
- Not yet verified on live Amazon searches

### Badge filters
- All four badge filter checkboxes render as vertical stack below price range row
- Results summary line now updates to show "N hidden by badge filter" when active (v0.6.1.44)

### Kill switch (v0.6.1.45)
- Extension fetches `https://actuallyuseful.net/killswitch.json` on every load (cache: no-store)
- If `disabled: true`, shows a red banner and halts — no extension runs
- Custom `message` field displayed in the banner
- Fail-open: network errors and timeouts (3s) are silently ignored; extension runs normally
- To activate: edit killswitch.json in repo, set `"disabled": true`, push to GitHub. Takes effect for all users within minutes.
- `manifest.json` host_permissions now includes `https://actuallyuseful.net/*`
...
- PPU inference notes shown as italic muted text below PPU value
...
- **Rice solid product override firing incorrectly** — "15 lbs (Pack of 2)" triggering solid override; some edge cases remain
...
- **No selector resilience** — single-string CSS selectors throughout search.js; Amazon changes break silently
- **No self-test mode** — extension cannot detect its own breakage
- **No welcome page on install** — chrome.runtime.onInstalled not yet wired; first impression is silence
...
- **Always provide a commit message when a GitHub push is needed** — standing rule
- **Don't conflate "small code change" with "well-understood problem"** — weight unit bugs proved complexity hides in Amazon's data
- **End-of-session checklist:**
  1. Project_Briefing.md updated
  2. Roadmap.md updated
  3. All changed code/website files presented
  4. If code changed: GitHub push reminder given + commit message provided
...
**Script delivery:** targeted `str_replace` edits on existing file — not full rewrites. Working file lives at `/tmp/search_new.js`. Copy to `/home/runner/workspace/search.js` for packaging. Always package as `.tar.gz`.

**Confirm before coding.** Always align with Melissa on what we're building before touching any files.

**One decision surface per session.** Polish items together = one surface. Architecture change = one surface.

**Context rot warning.** Long sessions degrade quality. Stop and wrap up rather than pushing through.

**Template literal rule:** Never use Python heredoc string escaping to write JavaScript template literals — produces `\'` sequences invalid in .html files. Use string concatenation (`+`) for all HTML-building JS in compare.html.
...
**Rollback rule:** If three attempts at fixing a bug fail in one session, stop. Roll back to the last stable commit.
