...
**Owner:** Melissa, retired, Seattle. Uses Chrome (butactuallyuseful profile) for primary testing. Has fibromyalgia causing brain fog and reduced memory — patience, thoroughness, and clear step-by-step instructions are essential. Produces complete updated document files at end of session rather than snippets or chunks — Melissa cannot reliably copy/paste multiple targeted edits.
...
**Shortlist item object shape sent to compare.html (current — v0.6.1.46):**
```
...
- **Overall / canonical version:** v0.6.1.46 (search.js number — the main file)
- **Per-file versions:** search.js v0.6.1.46 · core.js v0.6.1.46 · compare.html v0.6.1.30 · background.js v0.6.1.16 · manifest v0.6.1
...
| `content/search.js` | Search results page panel (v0.6.1.46) — single file, all logic |
...
## 9. Key Features (confirmed in search.js v0.6.1.46)
...
### Telemetry
Sent via background.js. User can opt out via popup. Usage log: Google Sheet (46 columns as of v0.6.1.46). Apps Script sheet ID: 1EmTXKDTISyLG4T1k6TiDqeXYisffobTClStK8y47MXU.
...
- **compare.html logging** — no logging on compare page yet; planned next session
...
| Chrome Web Store | Published unlisted — approved April 2026 |
| Usage log | Google Sheet ID: 1EmTXKDTISyLG4T1k6TiDqeXYisffobTClStK8y47MXU — 46 columns as of v0.6.1.46 |
| Apps Script | Deployed as Version 2 — edit via butactuallyuseful Chrome profile → Google Sheet → Extensions → Apps Script |
| Feedback form | https://forms.gle/XU8RpYM3cGFTwQQ86 |
| Ko-fi | ko-fi.com/butactuallyuseful |
...
- **Produce complete updated document files at end of session** — not snippets or chunks. Melissa cannot reliably copy/paste multiple targeted edits. She can handle one clearly spelled out targeted change (e.g. a version number or a single changelog entry), but not several at once.
- All Google tasks: use butactuallyuseful Chrome profile
...
- **End-of-session checklist:**
  1. Project_Briefing.md updated — full file produced for download
  2. Changelog.md appended — full file produced for download
  3. Roadmap.md updated — full file produced for download
  4. Handover.md written — full file produced for download
  5. bug-test.md updated if applicable — full file produced for download
