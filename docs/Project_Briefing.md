...
**Shortlist item object shape sent to compare.html (current — v0.6.1.36):**
```
...
- **Solid product override:** when title contains pod/sheet/strip/load/pac/fling/tab/toothpaste/tooth paste keywords and Amazon reports a weight unit or whole-package $/ct, calculates price/count instead
...
- **Sub-penny PPU:** 3 decimal places when PPU ≤ $0.01 (e.g. $0.009/ct)
...
- **Page limit confirmed by testing (Chat 42):** 7 pages maximum for high-density searches (~48 results/page); up to 10 pages for low-density searches (~16 results/page). Amazon is the ceiling.
- **Slider max → 7 pending** — code change not yet made; currently still shows 10
...
- **Cat food oz/lb inconsistency** — oz and lb results appear in same search with no normalization; can't be directly compared. Deferred pending weight design session.
...
- **Slider max still 10** — should be 7; code change pending
...
| GitHub Pages | tibbalsgribbin.github.io/actually-useful/ (live) |
| Custom domain | actuallyuseful.net → GitHub Pages ✅ (Chat 42); HTTPS enforcement pending SSL |
...
- **Don't conflate "small code change" with "well-understood problem"** — weight unit bugs proved complexity hides in Amazon's data, not just the code
