...
*Updated May 4, 2026 (Chat 48 — Brand filter Session 1)*
...
**Shortlist item object shape sent to compare.html (current — v0.6.1.48):**
```
...
- **Overall / canonical version:** v0.6.1.48 (search.js number — the main file)
- **Per-file versions:** search.js v0.6.1.48 · core.js v0.6.1.46 · compare.html v0.6.1.30 · background.js v0.6.1.16 · manifest v0.6.1
...
| `content/search.js` | Search results page panel (v0.6.1.48) — single file, all logic |
...
| `data/brand_blocklist.txt` | Bundled list of known-bad brands (70 entries, Chat 48) — wire-up in Session 3 |
...
**Background.js** fires a POST to the Apps Script endpoint on every panel load. Sheet has 46 columns as of v0.6.1.46. Brand filter fields (~18 new columns) will be added in Session 2.
...
Full design in Brand_Filter_Design.md. Session 1 complete.

**Detection layers (priority order):**
1. Bundled blocklist (`extension/data/brand_blocklist.txt`) — known junk, always flagged. Wire-up in Session 3.
2. Heuristic detector (`detectGibberishBrand`) — 5 signals. signalFakeMashup and signalAllCapsInvented flag alone; others require score ≥ 2.
3. Bundled allowlist (`extension/data/brand_allowlist.txt`) — known good, never flagged. Created in Session 3.
4. Personal blocklist (`chrome.storage.local`) — user-curated, always flagged. UI in Session 3.

**Console logging active** — `[AU brand] "OUGES" → signals: [...] score:1 flagged:true` — remove when UI ships in Session 2.

**brand and brandFlagged** are on every scraped item object as of v0.6.1.48. **brand** is in the compare payload (forward-compat; compare.html ignores it for now).
