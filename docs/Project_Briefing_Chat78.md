...
*Volatile state last updated: Chat 78 (Phase 7A complete — see PART TWO for current versions)*
...
*Phase 7A bundle (extension fixes + bug reporting) complete as of Chat 78.*

---

## 14. Current versions (as of Chat 78)
...
*(Phase 7B will update index.html, welcome.html, and privacy.html — next session.)*
...
**Copy status:** All user-facing copy on welcome.html and wizard marked `<!-- SUGGESTED COPY: ... -->`. Loading banner text in search.js marked `// <!-- SUGGESTED COPY: ... -->`. Not locked in.
...
- `sponDemotedHtml` bucket in `render()`. Items with `isSponDem` flag route there; appended after main list with a grey "N sponsored items moved to end" divider. Visual treatment matches existing amazon/brand dividers.

**Footer link consistency (v0.6.2.0):**
- All footer links normalized to 11px / `#c2362a` / no underline at rest / underline on hover.
- Inline styles removed from `#ppu-blocklist-link` and `#ppu-settings-link`; CSS-driven now.
...
- Phase 6 brand hint IIFE removed entirely from search.js.
- `dismissBrandHint` placeholder removed.
- Brand hint re-injection block removed from `render()`.
- `#ppu-hint-slot` div removed from `buildPanel()` HTML.
- All brand hint CSS removed from styles.css.
- `hasSeenBrandHint` variable, `auHasSeenBrandHint` storage key, and `loadPhase6Flags` retained — harmless; may be reused if Pattern A+B reuses the gate flag.

**Bug reporting — footer link + "click an item to report" flow (Chat 78):**
- `reportBtn` removed from `openBrandPopover()`. ⋯ popover is now two items: Always show / Always hide.
- `.ppu-brand-popover-item--report` CSS removed.
- `<span id="ppu-report-link">Report an issue</span>` added to footer (between My brand rules and Settings).
- `reportMode` boolean state variable added to buildPanel scope.
- `enterReportMode()`: sets `reportMode = true`, inserts `#ppu-report-banner` (sticky, coral) at top of `#ppu-scroll-area`, adds `.report-mode-target` class to all `.ppu-row` elements, wires Cancel button and ESC key.
- `exitReportMode()`: clears `reportMode`, removes banner, removes `.report-mode-target` from all cards, removes ESC listener.
- Capturing click listener on `#ppu-scroll-area` (fires before card-level handlers): if `reportMode`, extracts ASIN from clicked row, calls `exitReportMode()`, then `openBugReportOverlay(item)`.
- `openBugReportOverlay(item)` and `submitBugReport()` — unchanged. Only entry point changed.
- Banner copy flagged: `// <!-- SUGGESTED COPY -->` — "Click an item to report an issue."

**CSS added (styles.css, Chat 78):**
- `#ppu-report-link` — matches footer link pattern
- `#ppu-report-link:hover`
- `#ppu-report-banner` — sticky, coral surface, flex row
- `#ppu-report-cancel` — inline underline button
- `#ppu-report-cancel:hover`
- `.report-mode-target` — `box-shadow: inset 0 0 0 2px #fcc8c3; cursor:pointer`
- `.report-mode-target:hover` — `box-shadow: inset 0 0 0 2px #c2362a`

### What's deferred from Phase 7A

- **Pattern A+B** (`(?)` icons + Help drawer) — see Pattern_AB_Note.md; future phase
- **Panel_Redesign_Spec.md** — §8.3 and §5.7 brand-row mention are stale after Phase 7A decisions; update in a separate careful pass
- **"Always hide" semantics** — demotes instead of hides; pre-existing UX question

---

## Panel Redesign — phase status (as of Chat 78)
