...
*Volatile state last updated: Chat 80 (Phase 7B pushed; pre-Phase-8 research plan locked)*
...
- **PART TWO — VOLATILE STATE** (§14–§19, §21–§24). Current code state, current version numbers, phase-specific implementation notes. Rewritten at the end of each phase bundle.
...
**Known gaps under research (Chat 80):** multi-word compound brand truncation (Amazon Basics → "Amazon"; Arm & Hammer → "Arm"; 9 Elements missed); category-wide byline-selector failure (Asus, Logitech on keyboards; laptops entirely). Brand research session pending. See PART TWO §24.
...
*Phase 7 bundle (extension fixes + bug reporting + website polish) complete and pushed as of Chat 79. Pre-Phase-8 research plan opened Chat 80.*

---

## 14. Current versions (as of Chat 80)

- **Overall / canonical version:** v0.6.2.0 (search.js)
- **manifest:** v0.6.1
- **search.js:** v0.6.2.0
- **core.js:** v0.6.1.53
- **background.js:** v0.6.1.18
- **styles.css:** updated Chat 78
- **welcome.html:** updated Chat 79, pushed
- **welcome-bridge.js:** new Chat 75 (unchanged)
- **compare.html:** updated Chat 66 (unchanged)
- **index.html:** updated Chat 79, pushed
- **privacy.html:** updated Chat 79, pushed

---

## 15. Website pages — current state (as of Chat 80)

- `index.html` — pushed Chat 79: laundry pods (id=73) and laptop (id=74) sample compare links added; TODO comment re: screenshot; affiliate disclosure present
- `privacy.html` — pushed Chat 79: CSS variable names cleaned up; Bug reports section added (SUGGESTED COPY); affiliate disclosure present
- `compare.html` — palette migrated to coral+slate Chat 66; unchanged since. Structural pass scheduled as Phase 8 (after brand research and notes design sessions).
- `welcome.html` — pushed Chat 79: 02 Narrow copy updated; 03 Decide copy (Melissa's exact wording); brand explainer reframed (SUGGESTED COPY); alpha/dev notice added before CTAs (SUGGESTED COPY)

**SUGGESTED COPY blocks requiring Melissa review before CWS push:**
- welcome.html — headline, tagline, prologue, feature 01, all wizard screens, brand explainer, alpha/dev notice, wizard done state
- search.js — banner text in `enterReportMode()` — "Click an item to report an issue."
...
**Phase 8 will revisit:** structural pass plus bug reporting on compare.html. Phase 8 scope finalized after pre-Phase-8 research lands (see §24).
...
**Brand row:** plain text (`.ppu-brand-name`, clickable — opens same popover as ⋯) + ⋯ menu button (`.ppu-brand-menu-btn`). Clicking either opens popover: "Always show [brand]" / "Always hide [brand]". Row hidden if no brand detected. Brand name has `cursor:pointer` and coral hover state.
...
**Welcome content sections (as of Chat 79, pushed):**
1. Headline + tagline
2. Step 0 prologue callout (left-border accent)
3. Three feature cards: 01 Expand / 02 Narrow / 03 Decide
4. Brand controls explainer with visual mockup (popover shows 3 items including "Hide this seller (coming soon)" — signals roadmap; HTML comment in code notes this)
5. Privacy/telemetry toggle (defaults on; reads/writes `au_telemetry_enabled`)
6. Alpha/dev notice (new Chat 79 — `.prologue` class, SUGGESTED COPY)
7. Two CTAs: "Get started" (scrolls to wizard) · "Skip and start shopping" (amazon.com)
...
| `auHasSeenBrandHint` | boolean | `false` (key retained; no longer used by any active code) |
...
**Copy status:** All user-facing copy on welcome.html and wizard marked `<!-- SUGGESTED COPY: ... -->`. Loading banner text in search.js marked `// <!-- SUGGESTED COPY: ... -->`. Not locked in — Melissa reviews before CWS push.

---

## 22. Phase 7A — Extension fixes + bug reporting (Chats 76–78)

*v0.6.2.0. Complete as of Chat 78.*

### What shipped

**Sponsored items move to end (v0.6.2.0):**
- `sponDemotedHtml` bucket in `render()`. Items with `isSponDem` flag route there; appended after main list with a grey "N sponsored items moved to end" divider.

**Footer link consistency (v0.6.2.0):**
- All footer links normalized to 11px / `#c2362a` / no underline at rest / underline on hover.
- Footer link order: Give feedback · Buy me a coffee · My brand rules · Report an issue · Settings (five links).

**Keyword hint selectable (v0.6.2.0):**
- `.ppu-kw-hint` has `user-select:text; cursor:text`.

**Unit pill size reduction (v0.6.2.0):**
- Font-size 12px → 10px, padding 3px 10px → 2px 7px.

**Slider tick contrast (v0.6.2.0):**
- Color changed from `var(--au-border)` to `#b0b0bc`. Major ticks 5px → 6px, minor 3px → 4px.

**Brand name clickable (v0.6.2.0):**
- Click listener on `.ppu-brand-name` opens same popover as ⋯. `cursor:pointer` + coral hover state added.

**Brand hint stripped (Chat 78):**
- Phase 6 brand hint IIFE and all related HTML/CSS removed entirely.
- `hasSeenBrandHint`, `auHasSeenBrandHint`, and `loadPhase6Flags` retained — harmless; may be reused by Pattern A+B.

**Bug reporting — footer link + "click an item to report" flow (Chat 78):**
- ⋯ popover reduced to two items: Always show / Always hide.
- `<span id="ppu-report-link">Report an issue</span>` added to footer (between My brand rules and Settings).
- `reportMode` state + `enterReportMode()` / `exitReportMode()` / `onReportEsc()` functions.
- Capturing click listener on `#ppu-scroll-area` intercepts card clicks in report mode.
- Banner copy flagged `// <!-- SUGGESTED COPY -->` — "Click an item to report an issue."

### What's deferred from Phase 7A

- **Pattern A+B** (`(?)` icons + Help drawer) — see Pattern_AB_Note.md; future phase
- **Panel_Redesign_Spec.md** — §8.3 and §5.7 brand-row mention stale; separate careful pass
- **"Always hide" semantics** — demotes instead of hides; pre-existing UX question

---

## 23. Phase 7B — Website polish (Chat 79)

*Complete and pushed as of Chat 79. Phase 7 bundle closed.*

**welcome.html:**
- 02 Narrow: removed "Cut through 60 results" sentence. Boolean examples added as SUGGESTED COPY comment.
- 03 Decide: Melissa's exact wording — final, not SUGGESTED COPY.
- Brand explainer: reframed entirely. "Brand controls" heading. SUGGESTED COPY. Bug reporting reference corrected to footer link (not ⋯ menu). Popover visual retains 3 items to signal roadmap.
- Alpha/dev notice: new section before CTAs, `.prologue` class, SUGGESTED COPY.

**index.html:**
- Laundry pods (id=73) and laptops (id=74) sample compare links added.
- TODO comment added re: replacing id=72 googly eyes link once unit display verified.

**privacy.html:**
- Stale CSS variable names cleaned up (hex values unchanged).
- Bug reports section added: documents `bug_reports` Supabase table, what's sent, when, why. SUGGESTED COPY.

---

## 24. Pre-Phase-8 research plan (Chat 80)

*Opened Chat 80. Three sessions to run before Phase 8 code, in this order.*

### Why first

Phase 8 is the compare.html structural pass + bug reporting on compare.html. Brand is a column and a filter dimension on compare; unreliable brand detection makes compare look broken. Notes is a bigger component of compare than of the extension, and current behavior is poorly documented. Better to land both research outputs before designing Phase 8 itself.

### Session 1 — Brand research (Opus, research doc)

**Three questions:**

1. **What's actually getting scraped today?** For each failing case, which of `scrapeBrand()`'s three strategies fires (or do all return null)? Where does the real brand live in the DOM on those cards? Is the byline selector wrong, missing, or just absent on those cards?

2. **What can rules realistically fix?** Multi-word compound brands (Amazon Basics, Arm & Hammer, 9 Elements, Up & Up, Honest Co, etc.) — explicit list of compound names that should not be truncated by the first-word fallback. Category-specific byline-selector failures (laptops in particular) — different HTML pattern? The boundary between "fixable with rules" and "needs an external list."

3. **What external brand lists exist, and how would we use them?** Wikipedia category pages (typically clean and exhaustive). Open Brands / Wikidata. Mosley list (reconsider for broader use). USPTO trademark (probably too broad — note as researched-but-rejected if so). Amazon's `/stores/[Brand]/page/...` URLs (canonical names, but scraping is its own project). Decision criteria: license, size, freshness, false-positive rate.

**Deliverable:** research doc that proposes 2–3 viable approaches with tradeoffs. Decision at end: what becomes Phase 9 (or wherever brand work lands).

**Melissa brings:** Instant Data Scraper exports from 3–4 failing Amazon searches (laptops, keyboards, laundry detergent + one working category as control), current `search.js`, current `brand_blocklist.txt`, current `amazon_brands.txt`.

### Session 2 — Notes design (Opus, design doc)

**Current state to document:**
- Where notes are entered (extension panel? compare.html? both?)
- Where notes are stored (chrome.storage.local? shortlist memory only? Supabase row?)
- Whether notes persist when an item leaves the shortlist
- Whether notes are editable on compare.html and whether edits save back
- Character limits, line break behavior

**Open design questions:**
- **Where notes should live** — session-only / persist across browser close / live on compare.html only / both extension and compare synced
- **What notes are for** — quick reminders / comparison criteria / per-item vs. comparison-wide
- **Privacy** — notes are user-authored text. If they go to Supabase, they're stored on a third-party server. Public-by-obscurity (anyone with the ID URL can view). Personal info in notes is a real concern. Options: notes never sync / notes sync with warning / encrypted client-side / per-note share toggle.
- **Sharing** — compare URLs are shareable. If notes are in the row, they're shared automatically. Per-note public/private flag? Strip-on-share button? Two share URLs?

**Deliverable:** design doc covering current behavior, proposed behavior, storage, privacy, sharing UX.

**Melissa brings:** current `search.js`, current `compare.html`, anything else storage-related.

### Session 3 — Phase 8 kickoff brief (Opus)

Produced **after** brand research + notes design land. Phase 8 = compare.html structural pass + bug reporting on compare.html, informed by both outputs. Sonnet then executes.

### Text-size — parked separately

Not part of this research bundle. Friend with poor eyesight reports general issue with Ctrl+/- on overlays. AU panel observed to scale less per Ctrl-press than the underlying page. Decision pending Melissa's in-person observation of friend using AU. Outcome may be: fix Ctrl-zoom behavior on the panel, or design a Small/Medium/Large setting. Don't decide before observing.

---

## Panel Redesign — phase status (as of Chat 80)
...
- [x] **Phase 7A — Extension fixes + bug reporting** (Chats 76–78) ✅
- [x] **Phase 7B — Website polish** (Chat 79) ✅ — pushed
- [ ] Brand research session (Opus) — pre-Phase-8
- [ ] Notes design session (Opus) — pre-Phase-8
- [ ] **Phase 8 — compare.html structural pass + bug reporting on compare.html** (design TBD, after research)
