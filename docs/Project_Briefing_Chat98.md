# Project Briefing — Chat 98

*Updated end of Chat 98 Opus session. Phase 2 taxonomy complete; `Phase2_Taxonomy.md` groups 15 in-scope catalog collisions into 8 shapes organized by detection signature. Verification queue established. No code changes.*

*Two parallel work streams active: Phase 8B residue (unchanged since Chat 92 — no testing or code work in Chats 93–98) and the unit-collision design track (Phase 2 done as of this session).*
...
No Phase 8B work in Chats 93–98 (design-track sessions).

### Track 2 — Unit-collision design (active)

Phase 1 catalog (`Unit_Catalog_Phase1.md`) completed Chat 93. Trust posture framework discussed and drafted in Chat 94, audited in Chat 95, consolidated in Chat 96. Phase 1.5 catalog tagging done Chat 97. Phase 2 taxonomy done Chat 98 — 8 shapes covering 15 in-scope entries, organized by detection signature with posture as a secondary axis. Verification queue established.
...
- `Phase2_Taxonomy.md` — 8 shapes (A–H) covering the 15 in-scope catalog collisions; verification queue for Phase 3 prep.

Next phases: Phase 3 (detection rules, gated on verification-queue narrowing), Phase 4 (ambiguity-note redesign).
...
## Phase 2 taxonomy — eight shapes (Chat 98)
...
Phase 3 entry conditions: shapes stable; Shape D's split accepted as framework behavior; verification queue narrowed by at least one round of Amazon searches.
...
**Not yet tested:** columns, filters, sort, cross-tab sync, merge-gap fix, no-bridge fallback. No testing in Chats 93–98.
...
| **Phase 2 taxonomy** | **`Phase2_Taxonomy.md` (Chat 98). See section above.** |
...
| Which categories actually behave like toothpaste (Shape F verification) | Phase 3 prep verification |
| Whether SPECULATIVE catalog entries hold up under verification | Phase 3 prep verification |
...
| `Phase2_Taxonomy.md` | **New Chat 98** | 8 shapes covering 15 in-scope entries; verification queue for Phase 3 prep |
...
1. **Track 2 (design) — Chat 99**: Phase 3-prep verification (recommended) — Amazon searches against highest-priority Shape A and Shape F candidates from the `Phase2_Taxonomy.md` verification queue. Mode is observational and interactive.
2. **Alternative — Phase 3 directly**, if verification feels premature. Risks under-specifying detectors.
