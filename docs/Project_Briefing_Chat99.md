# Project Briefing — Chat 99

*Updated end of Chat 99 Opus session. Phase 3-prep verification — first round complete. Two Shape A speculative entries verified against live Amazon searches; both disconfirmed the predicted Override-suppress failure mode. `Unit_Catalog_Phase1.md` and `Phase2_Taxonomy.md` updated with verification notes; new "non-failure outcome patterns" (N1, N2) added to taxonomy cross-shape section. No code changes.*

*Two parallel work streams active: Phase 8B residue (unchanged since Chat 92 — no testing or code work in Chats 93–99) and the unit-collision design track.*
...
- `Phase2_Taxonomy.md` — 8 shapes (A–H) covering the 15 in-scope catalog collisions; verification queue for Phase 3 prep. Updated Chat 99 with Shape A verification findings and N1/N2 non-failure patterns.
- `Unit_Catalog_Phase1.md` — updated Chat 99 with verification notes on sinkers and fishing line entries.

Next phases: Phase 3 (detection rules, gated on verification narrowing — more verification rounds needed first), Phase 4 (ambiguity-note redesign).
...
**Added Chat 99:** Non-failure outcome patterns (N1, N2) — categories where Amazon already handles the case acceptably, so no detector is needed. N1 = Amazon omits PPU (sinkers example). N2 = Amazon recategorizes from a different unit in the title (fishing line → $/foot). Working note in `Phase2_Taxonomy.md` Cross-shape patterns section, not framework-level.

Phase 3 entry conditions: shapes stable; Shape D's split accepted as framework behavior; verification queue narrowed by at least one round of Amazon searches. **Round 1 done Chat 99 (2 of many entries). Round 2 still needed before Phase 3.**

---

## Phase 3-prep verification — round 1 results (Chat 99)

Two Shape A speculative entries verified against live Amazon (extension off, scraped via Melissa's existing extension).

| Entry | Predicted | Actual | Pattern |
|---|---|---|---|
| Fishing sinkers oz | Override-suppress needed | Amazon omits PPU | N1 |
| Fishing line lb test | Override-suppress needed | Amazon computes $/foot from spool length | N2 |

Both verifications: single search, first-page natural sort. Catalog status remains SPECULATIVE pending broader confirmation. VERIFIED Shape A entries (paper grade lb, dumbbells lb) unaffected.

**Implication:** Phase 3 detector design for these specific sub-patterns is downgraded in priority. The verification queue's job is partly to distinguish "needs a detector" from "Amazon already handles it" — a distinction the original taxonomy did not anticipate.
...
| Which categories actually behave like toothpaste (Shape F verification) | Phase 3-prep verification round 2 |
| Whether SPECULATIVE catalog entries hold up under verification | Phase 3-prep verification (round 1 surfaced N1/N2 non-failure outcomes; more rounds needed) |
| Whether N1/N2 outcome patterns warrant a formal catalog status tag | Open — depends on whether further verification keeps surfacing these outcomes |
...
| `Unit_Catalog_Phase1.md` | Updated Chat 99 | Verification notes added to sinkers oz and fishing line lb test entries |
| `Phase2_Taxonomy.md` | Updated Chat 99 | Shape A verification findings added; Cross-shape patterns gained N1/N2 note; queue updated |
...
## Deferred from Chat 99

- **Promotion of N1/N2 outcome patterns.** Currently a working note in `Phase2_Taxonomy.md` Cross-shape patterns. Promote to a formal catalog status tag (e.g. "VERIFIED non-collision — Amazon handles via N1/N2") only after at least one more verification round surfaces the same outcomes. Hold until Chat 100 or later.

---

## What's next

1. **Track 2 (design) — Chat 100**: Phase 3-prep verification round 2 — at least one more Shape A subtype (boxing gloves, weighted vest, or similar non-fishing weight-class entry) and the first Shape F verification (semi-solid personal care or pourable food). Mode remains observational.
2. **Alternative — Phase 3 directly**, if verification feels premature. Risks under-specifying detectors that need verification input.
