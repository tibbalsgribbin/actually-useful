...
## Current version: v0.6.1.78 (overall) · v0.6.1 (manifest) · v0.6.1.78 (search.js) · v0.6.1.53 (core.js) · compare.html updated Chat 61 · v0.6.1.17 (background.js) · styles.css updated Chat 60 · welcome.html created Chat 59
...
**Version numbering:**
- Overall / canonical: v0.6.1.78 (search.js number)
...
1. **$/serving for protein powder — design session** — Amazon reliably lists "N Servings" in the brand row for protein powder. $/serving alongside $/oz would enable comparison by serving count. altPPU/altUnit fields already exist. Design the display and detection logic before coding.
...
- [x] isPaperWeightLb() helper — centralises paper-weight lb detection (Chat 62)
- [x] titleHasWeightQty — paper-weight lb no longer triggers weight path (Chat 62)
- [x] Weight-from-title fallback — lbM2 suppressed when isPaperWeightLb fires (Chat 62)
- [x] scrapeBrand Strategy 3 — expanded adjective blocklist (Chat 62)
- [x] extractCount — N/Pack pattern added (Chat 62)
- [x] isMultiPackWeight() helper — guards weight × pack count multiplication (Chat 63)
- [x] isServingWeight() helper — suppresses per-serving nutrition gram values (Chat 63)
- [x] Multi-pack × weight PPU — count&&price branch and weight-from-title fallback (Chat 63)
- [x] Oz hyphen fix — [- ]* replaces \s* in all three oz extraction regexes (Chat 63)
