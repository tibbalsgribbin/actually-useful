...
- **Auto-resort on Re-sync page-add** — not yet verified
...
- **Apps Script header row** — needs updating from 56 to 58 columns (2 personal blocklist fields added Chat 50)
- **Brand filter threshold tuning** — 25% high-noise threshold needs real-world calibration
- **AU_VERSION in core.js stale** — still 0.6.1.46; logged scriptVersion doesn't reflect actual version. Bump next session.
- **Brand filter allowlist** — deferred; add after telemetry shows false positives worth addressing
...
**Version numbering:**
- Overall / canonical: v0.6.1.50 (search.js number)
...
1. **Bump AU_VERSION in core.js** to 0.6.1.50 (or whatever the current session produces) — do this at the start of next session
2. **Brand filter — Session 4: delivery window filter** — checkbox + slider, filter logic using earlier of freeDateTs/fastDateTs, hide/demote toggle (default hide), results summary, persist state, logging fields. Files: search.js, styles.css. See Brand_Filter_Design.md Session 4 scope.
3. **compare.html logging** — direct fetch to Google Sheets endpoint; deferred from Chat 46
...
- [ ] AU_VERSION bump in core.js — next session
- [ ] Apps Script header row updated to 58 columns
...
- [x] Usage log — Google Sheet, 58 columns (header row update pending) ✅
...
- [ ] Brand filter feature suite (Sessions 1–3 complete — see v0.7 section)
...
- Walmart version
- Brand filter allowlist — add after telemetry shows false positives worth addressing
