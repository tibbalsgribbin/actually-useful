...
- **Brand column blank for electronics** — search.js scrapeBrand() first-word fallback grabs purchase metadata ("50+ bought in past month") when no brand byline on card. compare.html isBrandNoise() rejects these correctly; root fix needs search.js work.
