...
## Current version: v0.6.2.0 (overall) · v0.6.1 (manifest) · v0.6.2.0 (search.js) · v0.6.1.53 (core.js) · v0.6.1.18 (background.js) · styles.css updated Chat 78 · welcome.html pushed Chat 79 · welcome-bridge.js new Chat 75 · compare.html updated Chat 66 · index.html pushed Chat 79 · privacy.html pushed Chat 79
...
- **Brand filter — multi-word compound brand truncation** — Amazon Basics → "Amazon"; Arm & Hammer → "Arm"; 9 Elements not detected. First-word-of-title fallback firing where byline should win. To address in brand research session.
- **Brand filter — category-wide byline-selector failures** — Asus, Logitech not detected on keyboard searches; zero brands detected on laptop searches. Likely different HTML pattern on those categories. To address in brand research session.
...
- **Amazon Basics brand column shows — on compare.html** — needs investigation. (Likely tied to truncation issue above.)
...
- **Text-size — friend feedback** — friend with poor eyesight reports overlays in general scale worse than underlying pages with Ctrl+/-. Melissa confirms AU panel scales less per Ctrl-press than underlying page. Needs in-person observation session before any design or build.
- **Notes — current behavior poorly documented** — `note` field exists in shortlist payload but storage, edit-back, sharing, and privacy semantics not clear. Promoted to pre-Phase-8 design item.
...
- Pre-Phase-8 research bundle (brand research + notes design + Phase 8 kickoff brief) → in progress, opened Chat 80
...
4. GitHub commit message + push reminder (if coding OR if any project doc changed)
...
**Brand research session (Opus).** Research doc deliverable. Three questions to answer:

1. What's actually getting scraped today on failing searches?
2. What can rules realistically fix (compound brand names, category-specific selector failures)?
3. What external brand lists exist and how would we use them?

Melissa to bring: Instant Data Scraper exports from 3–4 failing searches (laptops, keyboards, laundry detergent + working control), current `search.js`, `brand_blocklist.txt`, `amazon_brands.txt`.

Then **Notes design session (Opus)** — design doc covering storage, privacy, sharing, UX.

Then **Phase 8 kickoff brief (Opus)** — compare.html structural pass + bug reporting on compare.html, informed by both research outputs.

Then **Phase 8 execution (Sonnet)** — from the brief.
...
- [ ] **Brand research session** (Opus, research doc — pre-Phase-8)
- [ ] **Notes design session** (Opus, design doc — pre-Phase-8)
- [ ] **Phase 8 — compare.html structural pass + bug reporting on compare.html** (design TBD after brand + notes research)
- [ ] **Text-size — friend observation session** (deferred until in-person observation possible)
...
- [x] Website polish ✅ (Phase 7B — Chat 79, pushed)
...
- [ ] Brand detection improvements (post-research)
- [ ] Notes behavior locked and documented (post-design)
...
- Text-size setting — design only after in-person observation with affected user
