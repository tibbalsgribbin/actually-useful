...
*Updated end of Chat 97. Phase 1.5 catalog posture tagging complete. Twenty entries in `Unit_Catalog_Phase1.md` carry inline posture tags (13 firm, 7 hypothesis). `bug-test.md` toothpaste verdict reconciled. No code changes.*
...
2. **Unit-collision design** — Phase 1 catalog (Chat 93), trust posture framework (Chat 96), Phase 1.5 catalog tagging (Chat 97, done), Phase 2 taxonomy, Phase 3 detection rules, Phase 4 ambiguity-note redesign. Opus design work; eventual Sonnet implementation.

---

## Immediate — next session (Chat 98)

**Track: unit-collision design.**

### Phase 2 — taxonomy kickoff (recommended)

Group verified collisions by shape. Each shape maps to one or more postures. Phase 1.5 feeds in: every VERIFIED catalog entry now carries a posture tag, so the shape→posture analysis can use the catalog directly as input.

Opus session. Design-judgment work; Phase 1.5 lowered the per-entry overhead but the synthesis step is novel.

### Alternative — Phase 2 prep / SPECULATIVE verification

If Phase 2 feels too big for one session, an intermediate move: take 2–4 catalog SPECULATIVE entries with `posture-hypothesis` tags and run verification searches against Amazon. Pick ones likely to clarify shape boundaries. Outputs feed Phase 2 with firmer postures.
...
Carried forward from Chat 92 Roadmap; not touched in Chats 93–97 (design-track sessions).
...
| Phase 2 — Taxonomy | Not started | Group verified collisions by shape. Next session candidate. |
