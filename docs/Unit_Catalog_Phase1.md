...
* `ITEM\_UNITS` deliberately excludes weight and liquid units so they fall through to a weight-context check rather than being accepted blindly.
...
From `LIQUID\_UNITS`, `WEIGHT\_UNITS`, `CONTAINER\_UNITS`, `LENGTH\_UNITS`, `ITEM\_UNITS`, plus regex tokens inside `guessCountUnit()` and `extractCount()`.
...
* **SPECULATIVE — Boxing/MMA glove weight class.** "16 oz gloves." Spec of the glove, not quantity. *Verification note (Chat 100, May 21 2026): single search for "boxing gloves" showed Amazon reports NO PPU for any glove listing (pattern N1, third instance). AU computes $/oz from the title weight ("8 oz", "12 oz", "16 oz") and carries the note "ℹ No Amazon unit price — calculated from weight in title." This is exactly the Shape A collision predicted, sourced by AU rather than Amazon. Status remains SPECULATIVE pending broader confirmation. See Phase2\_Taxonomy.md Shape A verification findings for the three-instance pattern.*
* **SPECULATIVE — Fishing weights/sinkers.** "1 oz lead sinker." Spec. *Verification note (Chat 99, May 21 2026; updated Chat 100, May 21 2026): single search for "fishing sinkers" showed Amazon reports NO PPU for any sinker listing (pattern N1, see Phase2\_Taxonomy.md). However, Chat 100 re-check with AU extension on showed AU computes $/oz for 36 of 178 listings, extracting the oz from the title spec — exactly the Shape A collision predicted. The Amazon-level absence of PPU is real, but it does not prevent the collision because AU's own detectors produce $/oz independently. Status remains SPECULATIVE pending broader confirmation, but Phase 3 detector work for this entry is needed regardless of Amazon's behavior.*
...
* **SPECULATIVE — Fishing line test strength.** "20 lb test braided line." Spec. *Verification note (Chat 99, May 21 2026; updated Chat 100, May 21 2026): single search for "braided fishing line" showed Amazon computes $/foot from spool length, not $/lb (pattern N2, see Phase2\_Taxonomy.md). However, Chat 100 re-check with AU extension on showed AU computes $/lb for 37 of 162 listings, extracting the lb from the title test rating — exactly the Shape A collision predicted. Amazon's recategorization to $/foot is real, but it does not prevent the collision because AU's own detectors produce $/lb independently. Status remains SPECULATIVE pending broader confirmation, but Phase 3 detector work for this entry is needed regardless of Amazon's behavior.*
...
* **VERIFIED — "in" as preposition risk.** Literal token `'in'` in `LENGTH\_UNITS` is risky for false matches in titles like "best laptops in 2026." No specific bug logged but the risk is plain.
...
* **`yard` of fabric** — already in LENGTH\_UNITS as a length, but the PPU users want for yarn is $/yard and the current panel doesn't surface it. *Surfaced from Melissa's yarn screenshot — pills offered were oz/lb/g/kg/per item/As listed, no yardage option.*
...
\## VERIFIED non-collisions (Shape F defer cases)



These entries document categories where AU's behavior has been verified as correctly deferring to Amazon's per-listing PPU choices. Unlike collision entries elsewhere in this catalog, these are explicit \*non-collision\* findings — the framework predicted defer, the verification confirmed defer. They are catalog-grade because for defer cases the verification \*is\* the confirmation. See `Phase2\_Taxonomy.md` Shape F "Verification findings (Chat 101)" subsection for full methodology.



These entries serve two purposes in Phase 3:



1\. \*\*Anchor list for defer-category detection.\*\* Phase 3 may want explicit category lists to suppress other detectors that would misfire on confirmed defer categories.

2\. \*\*Verification-coverage record.\*\* Future verification work knows what's been checked vs. what hasn't.



\### Deodorant (semi-solid personal care, uniform-PPU)
...
\*\*Verification (Chat 101, May 21 2026).\*\* Search: `deodorant`. Amazon shows \~uniform `(/ounce)` PPU across the search results (59/60 PPU lines on the scraped page; one outlier `(/count)` on a 2-pack item where ct = identical-unit count). AU produces unit families `oz(192), ct(24), fl oz(9), g(1)` across 226 listings, with selected unit `as-listed` and `Liquid Dominant` FALSE. AU correctly defers to Amazon's per-listing PPU choices.



\*\*Defer configuration:\*\* Uniform-PPU. Amazon makes the same unit choice across nearly all listings; AU follows.



\### Lotion (semi-solid personal care, mixed-but-coherent)
...
\*\*Verification (Chat 101, May 21 2026).\*\* Search: `lotion`. Amazon shows a mixed liquid-dominant PPU pattern across listings — \~48 `(/fluid ounce)` (liquid lotions), \~8 `(/ounce)` (semi-solid balms), \~3 `(/milliliter)` (imported / smaller products) on the 60-listing scrape. AU produces unit families `fl oz(123), oz(35), ct(9), ml(5), g(1)` across 174 listings, with selected unit `as-listed` and `Liquid Dominant` TRUE. AU correctly follows Amazon's per-product choices without trying to normalize across the mix.



\*\*Defer configuration:\*\* Mixed-but-coherent. Amazon makes different unit choices per product based on each product's form; AU follows each per-listing choice. The framework's defer prediction holds even when Amazon's per-listing PPU is non-uniform across the page.



\### Verification queue (Shape F non-collisions)



\- ✓ Deodorant — verified Chat 101.

\- ✓ Lotion — verified Chat 101.

\- Next: canned soup or canned beans (pourable/semi-solid food defer case).

\- Queued: other semi-solid personal care, other pourable foods, canned goods, pressurized cans.
