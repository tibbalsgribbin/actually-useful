...
// ── Affiliate tag ─────────────────────────────────────────────────────────
// Injected on all links generated by the companion. Never on Amazon's native nav.
// Leave empty until Associates account is approved.
const AU_AFFILIATE_TAG = '';

function auTagUrl(url) {
  if (!AU_AFFILIATE_TAG || !url) return url;
  try {
    var u = new URL(url);
    u.searchParams.set('tag', AU_AFFILIATE_TAG);
    return u.toString();
  } catch (e) { return url; }
}
